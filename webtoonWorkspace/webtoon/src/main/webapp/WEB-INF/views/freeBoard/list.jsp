...
<link rel="stylesheet" href="/webtoon/resources/css/freeBoard.css">
		...
		<div class="list">
			...
			<c:forEach items="${voList}" var="vo">
				<div class="list_num">
					...
					<div class="num_main">
						<a href="/webtoon/board/freeBoard/detail?no=${vo.no}">${vo.title}</a>
					</div>
			...
			<div class="page_area">
				<c:if test="${pvo.currentPage > 1}">
					<a href="/webtoon/board/freeBoard/list?pno=${pvo.currentPage-1}">이전</a>
				</c:if>
				<c:forEach begin="${pvo.startPage}" end="${pvo.endPage}" var="x">
					...
					<c:if test="${pvo.currentPage != x}">
						<a href="/webtoon/board/freeBoard/list?pno${x}">${x}</a>
					</c:if>
				</c:forEach>
				<c:if test="${pvo.currentPage < pvo.maxPage}">
					<a href="/webtoon/board/freeBoard/list?pno=${pvo.currentPage+1}">다음</a>
				</c:if>
			</div>
		</div>
	</main>
